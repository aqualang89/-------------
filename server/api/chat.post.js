...
export default defineEventHandler(async (event) => {
  ...
  try {
    // Каталог ищем с учётом контекста диалога, а не только по последней реплике.
    // Клиент часто уточняет («у меня травник с креветками») без ключевого слова товара —
    // поиск по одной фразе терял тему из прошлых сообщений, и бот говорил «нет в наличии».
    // Фото текстом не описываем — поиск только по тексту.
    const recentUserText = previousHistory
      .filter(h => h.role === 'user')
      .slice(-2)
      .map(h => h.content)
    const searchQuery = [...recentUserText, cleanText]
      .filter(Boolean)
      .join('. ')
      .replace(/\[(?:приложено|прислано) фото\]/g, '')
      .trim()
    products = searchQuery ? await findRelevantProducts(searchQuery) : []

    const catalogBlock = products.length
      ? 'Подходящие товары под этот запрос (это ВЫБОРКА из каталога, не весь ассортимент — если нужного тут нет, не утверждай что в магазине его нет вообще). Под товаром его характеристики из описания — опирайся на них и НЕ выдумывай цифры (мощность, габариты и т.п.), которых там нет. Используй артикулы и цены; ссылки в текст НЕ вставляй — клиент добавит в корзину через [CART_ADD:артикул]:\n' +
        products.map(p => {
          const base = `— ${p.name} (арт. ${p.article || 'нет'}) — ${Math.round(p.price).toLocaleString()} ₽`
          const desc = (p.description || '').trim()
          return desc ? `${base}\n  ${desc.slice(0, 600)}` : base
        }).join('\n')
      : 'По этому запросу точных совпадений в выборке нет. Это НЕ значит что товара нет в магазине — отвечай экспертно по сути, дай критерии выбора и при необходимости предложи уточнить наличие. Не говори категорично «нет в наличии».'
